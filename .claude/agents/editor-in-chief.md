...
4. **Effect-measurement gate** — when rung 3 classified a hunk substantive AND the change is of a measurement-obligated type ([runbook § When to Run](../operations/proposal-validation-runbook.md)), an accept transition must exist in the corpus before commit; otherwise hold the commit, revert (or isolate) the edit, and then run the measurement — that revert is a temporary isolation for the measurement and is re-applied on acceptance, since staged guideline text contaminates its own measurement (acceptance rule: step 6 below; per-type procedure: the runbook's three variants)
