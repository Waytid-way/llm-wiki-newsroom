...
**2-tier model**: RAG (synthesis layer + directory) = answer synthesis / Graph (entity·concept·source full text) = deep-link drill-down. The RAG corpus is kept small by excluding entity·concept bodies (0.93 MB as measured on the original large corpus — the live budget is printed by `python tools/export.py`), and the Core set is auto-selected within the limit by priority-greedy fill.
